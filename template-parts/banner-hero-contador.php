...
/**
 * Template part: Hero Banner Principal con Imágenes Rotativas
 * 
 * @param bool $mostrar_fondo - true: muestra imágenes rotativas, false: sin imagen de fondo
 * @param bool $text_white - true: texto en blanco (excepto título parte 1), false: texto negro
 */
...
// Definir clases de color según el parámetro
$text_color_class = $text_white ? 'text-white' : 'text-black';

// Leer valores del Customizer - TEXTOS
...
// Leer valores del Customizer - IMÁGENES Y CONFIGURACIÓN
$intervalo = get_theme_mod('banner_principal_intervalo', 5);
$imagen1_id = get_theme_mod('banner_principal_imagen1', '');
$imagen2_id = get_theme_mod('banner_principal_imagen2', '');
$imagen3_id = get_theme_mod('banner_principal_imagen3', '');

// Preparar array de imágenes
$imagenes = array();
if (!empty($imagen1_id)) {
    $imagenes[] = wp_get_attachment_image_url($imagen1_id, 'full');
}
if (!empty($imagen2_id)) {
    $imagenes[] = wp_get_attachment_image_url($imagen2_id, 'full');
}
if (!empty($imagen3_id)) {
    $imagenes[] = wp_get_attachment_image_url($imagen3_id, 'full');
}

// Si no hay imágenes personalizadas, usar imagen por defecto
if (empty($imagenes)) {
    $imagenes[] = get_template_directory_uri() . '/assets/images/image1.png';
}
...
$banner_id = 'banner-principal-' . uniqid();
?>

<!-- Hero Banner Principal -->
<section id="<?php echo esc_attr($banner_id); ?>"
    class="banner-principal-container relative min-h-[550px] md:h-[400px] lg:h-[550px] flex-shrink-0 bg-center bg-cover bg-no-repeat py-12 lg:py-0 overflow-hidden"
    data-mostrar-fondo="<?php echo $mostrar_fondo ? '1' : '0'; ?>"
    data-intervalo="<?php echo esc_attr($intervalo * 1000); ?>">

    <?php if ($mostrar_fondo && !empty($imagenes)) : ?>
        <!-- Slides de imágenes de fondo -->
        <?php foreach ($imagenes as $index => $imagen_url) :
            $is_active = ($index === 0) ? 'active' : '';
        ?>
            <div class="banner-principal-slide <?php echo $is_active; ?> absolute inset-0 w-full h-full bg-cover bg-center transition-opacity duration-1000"
                style="background-image: url('<?php echo esc_url($imagen_url); ?>'); <?php echo $is_active ? 'opacity: 1;' : 'opacity: 0;'; ?>">
            </div>
        <?php endforeach; ?>

        <!-- Barra de progreso vertical (lado derecho) -->
        <div class="banner-principal-progress-container absolute top-0 right-0 h-full w-[4px] bg-white bg-opacity-20 z-10">
            <div class="banner-principal-progress-bar absolute top-0 left-0 w-full bg-white bg-opacity-80 transition-none"
                style="height: 0%;"></div>
        </div>
    <?php endif; ?>

    <!-- Contenido del Banner -->
    <div class="container mx-auto px-[30px] sm:px-[60px] md:px-[90px] lg:px-[60px] xl:px-[120px] h-full relative z-20">
        <div class="flex flex-col lg:flex-row items-center justify-center lg:justify-start h-full gap-6 lg:gap-[20px] xl:gap-[24px]">
            ...
            <div class="flex flex-col justify-center items-center lg:items-start w-full lg:w-[450px] xl:w-[532px] max-w-[315px] sm:max-w-[400px] md:max-w-[500px] lg:max-w-none flex-shrink-0 gap-6 lg:gap-[28px] xl:gap-[33.44px]">
                ...
                <!-- Descripción DINÁMICA con color condicional -->
                <p class="self-stretch:text-left <?php echo $text_color_class; ?> font-sans text-[16px] md:text-[17px] lg:text-[16px] xl:text-[18px] not-italic font-medium leading-[24px]"
                ...
                <h1 class="self-stretch lg:text-left font-sans text-[28px] md:text-[34px] lg:text-[32px] xl:text-[40px] not-italic font-bold leading-[36px] md:leading-[44px] lg:leading-[44px] xl:leading-[52px]">
                    <!-- Parte 1: SIEMPRE naranja -->
                    <span class="text-[#F8A60E]"><?php echo esc_html($banner_titulo_parte1); ?></span>
                    <!-- Parte 2: color condicional (blanco o negro) -->
                    <span class="<?php echo $text_color_class; ?>"> <?php echo esc_html($banner_titulo_parte2); ?></span>
                </h1>
            </div>
            ...
            <div class="flex flex-col w-full lg:w-[500px] xl:w-[605.237px] max-w-[315px] sm:max-w-[400px] md:max-w-[500px] lg:max-w-none gap-4 lg:gap-0">
                ...
                backdrop-blur-[2px] shadow-sm"
                    ...
                    <div class="flex flex-row gap-[8px] md:gap-[12px] lg:gap-[12px] xl:gap-[15px] self-stretch">
                        <span class="<?php echo $text_color_class; ?> font-sans text-[19.45px] md:text-[28px] lg:text-[30px] xl:text-[36px] not-italic font-medium"
                            style="line-height: 12.967px; font-feature-settings: 'liga' off, 'clig' off;">
                            Ya somos
                        </span>
                        <span class="<?php echo $text_color_class; ?> font-sans text-[34.57px] md:text-[50px] lg:text-[54px] xl:text-[64px] not-italic font-bold counter-number animate-counter"
                    ...
                    <div class="flex flex-row items-center justify-start gap-[8px] md:gap-[20px] lg:gap-[24px] xl:gap-[30px] self-stretch text-center">
                        <span class="<?php echo $text_color_class; ?> font-sans text-[9.725px] md:text-[14px] lg:text-[15px] xl:text-[18px] not-italic font-medium whitespace-nowrap"
                            style="line-height: 12.967px; font-feature-settings: 'liga' off, 'clig' off;">
                            Ayúdanos a llegar a
                        </span>
                        <span class="<?php echo $text_color_class; ?> font-sans text-[21px] md:text-[30px] lg:text-[34px] xl:text-[40px] not-italic font-bold"
                            style="line-height: 12.967px;">
